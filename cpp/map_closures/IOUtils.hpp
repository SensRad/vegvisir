...
// POD helpers ---------------------------------------------------------------
template <class T>
inline bool writePod(std::ostream& os, const T& v) {
...
inline bool writeMat(std::ostream& os, const cv::Mat& m) {
  ...
  return bool(
      os.write(reinterpret_cast<const char *>(m.data), static_cast<std::streamsize>(bytes)));
}
...
inline bool readMat(std::istream& is, cv::Mat& m) {
  ...
  const size_t bytes = static_cast<size_t>(rows) * cols * m.elemSize();
  return bool(is.read(reinterpret_cast<char *>(m.data), static_cast<std::streamsize>(bytes)));
}

// generic map<K,V> helpers --------------------------------------------------
template <class K, class Writer>
inline bool writeMap(std::ostream& os, const std::unordered_map<K, typename Writer::Value>& m,
                     const Writer& w) {
...
inline bool readMap(std::istream& is, std::unordered_map<K, Value>& m, const Reader& r) {
  size_t n = 0;
  if (!readPod(is, n)) {
    return false;
  }
...
struct ConfigIO {
  ...
  bool operator()(std::ostream& os, const Value& c) const {
    return writePod(os, c.density_map_resolution) && writePod(os, c.density_threshold) &&
           writePod(os, c.sift_match_ratio) && writePod(os, c.lbd_min_line_length) &&
           writePod(os, c.lbd_match_ratio) && writePod(os, c.lbd_num_octaves) &&
           writePod(os, c.lbd_scale) && writePod(os, c.density_map_gamma) &&
           writePod(os, c.lbd_weight);
  }
  bool operator()(std::istream& is, Value& c) const {
    return readPod(is, c.density_map_resolution) && readPod(is, c.density_threshold) &&
           readPod(is, c.sift_match_ratio) && readPod(is, c.lbd_min_line_length) &&
...
struct DensityMapIO {
  ...
  bool operator()(std::istream& is, Value& d) const {
    ...
    double res = 0.0;
    cv::Mat grid;
    if (!readPod(is, lb_x) || !readPod(is, lb_y) || !readPod(is, res) || !readMat(is, grid)) {
      return false;
    }
