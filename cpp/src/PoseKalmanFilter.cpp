...
PoseKalmanFilter::PoseKalmanFilter() {
  ...
  Eigen::Vector3d position_variance(Q_POS_VARIANCE_XY, Q_POS_VARIANCE_XY, Q_POS_VARIANCE_Z);

  Q_ = Matrix6d::Zero();
  Q_.block<3, 3>(0, 0) = Eigen::Matrix3d::Identity() * Q_ANGLE_VARIANCE_DEFAULT;  // rotation
