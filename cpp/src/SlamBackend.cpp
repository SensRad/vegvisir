...
void SlamBackend::initialize() {
  ...
  for (const auto& [map_id, local_map] : vegvisir_.local_map_graph_) {
    ...
    if (!reference_poses.empty() &&
        reference_poses.find(static_cast<int>(map_id)) != reference_poses.end()) {
  ...
  if (reference_poses.empty()) {
    if (!vegvisir_.local_map_graph_.getAllIds().empty()) {
      keypose_optimizer_->fixVariable(vegvisir_.local_map_graph_.getAllIds().front());
    }
  } else {
  ...
  std::cout << "SLAM backend initialized optimizer with " << vegvisir_.local_map_graph_.size()
            << " keyposes" << std::endl;
}
...
void SlamBackend::preIntegrate(const Eigen::Matrix4d& T_odom_base,
  ...
  Eigen::Matrix4d T_map_base = vegvisir_.local_map_graph_.lastKeypose() * vegvisir_.current_pose_;
...
void SlamBackend::generateNewNode(const Eigen::Matrix4d& T_odom_base) {
  ...
  if (local_trajectory.empty()) {
    std::cerr << "generateNewNode: local trajectory is empty, skipping" << std::endl;
    return;
  }
...

  {
    ...
    uint64_t new_id =
        vegvisir_.local_map_graph_.finalizeLocalMap(vegvisir_.voxel_grid_, Mode::SLAM);

    // Add variable to optimizer for the new keypose + odometry factor
    if (keypose_optimizer_) {
      keypose_optimizer_->addVariable(new_id, vegvisir_.local_map_graph_.lastKeypose());

      Eigen::Matrix<double, 6, 6> information = Eigen::Matrix<double, 6, 6>::Identity();

      keypose_optimizer_->addFactor(new_id, query_id_u64, relative_motion, information);
    }
  }
