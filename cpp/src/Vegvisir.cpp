...
#include <pthread.h>
  ...
  Vegvisir::Vegvisir(const std::string& map_database_path, Mode mode, const VegvisirConfig& config)
    ...
    try {
      ...
      auto result =
          loadDatabase(map_database_path, *map_closer_, local_map_points_, require_exists);
  ...
  Vegvisir::~Vegvisir() {
    ...
    if (mode_ == Mode::SLAM && loop_closure_enabled_ && !map_database_path_.empty() &&
        map_closer_ && !database_saved_) {
  ...
  void Vegvisir::processLoopClosuresAsync(
      ...
      std::vector<Eigen::Vector3d> query_points_icp, Eigen::Matrix4d query_odom_base) {
