...
Vegvisir::Vegvisir(const std::string &map_database_path, Mode mode,
                   const VegvisirConfig &config)
    : config_(config), voxel_grid_(config_.voxel_size),
      tf_map_odom_(Eigen::Matrix4d::Identity()), mode_(mode),
  ...
  std::cout << "    Splitting distance (SLAM): "
            << config_.splitting_distance_slam << '\n';
  std::cout << "    Splitting distance (localization): "
            << config_.splitting_distance_localization << '\n';
  ...
  try {
    ...
    if (result.success && !map_closer_->getReferencePoses().empty()) {
      rebuildLocalMapGraph(local_map_graph_, map_closer_->getReferencePoses(),
                           map_closer_->getReferenceTimestampsNs(),
                           local_map_points_);
    }
...
void Vegvisir::update(const std::vector<Eigen::Vector3d> &points,
  ...
  // Stamp the initial keypose with the first point cloud's timestamp.
  if (!initial_timestamp_set_ && !local_map_graph_.empty()) {
    local_map_graph_.lastLocalMap().setKeyposeTimestampNs(timestamp_ns);
    initial_timestamp_set_ = true;
  }
...
void Vegvisir::processLoopClosuresAsync(
  ...
  if (closure_future_.valid() && closure_future_.wait_for(std::chrono::seconds(
...
std::pair<bool, Eigen::Matrix4d> Vegvisir::performICPRefinement(
  ...
  const icp::Result result = icp::IcpSvd::pointToPointICP(
      reference_points, query_points, initial_pose,
      config_.icp_refinement_voxel_size, config_.icp_max_iterations,
      config_.icp_convergence_criterion,
...
Vegvisir::fineGrainedOptimizationAndUpdateKeyposes() {
  auto pgo_result =
      runFineGrainedPGO(local_map_graph_, closures_, gnss_state_, config_);
