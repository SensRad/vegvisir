...
void Vegvisir::update(const std::vector<Eigen::Vector3d>& points,
  ...
  std::vector<Eigen::Vector3d> downsampled_points = voxel_map::voxelDownsample(points, VOXEL_SIZE);
...
void Vegvisir::processLoopClosures(int query_id,
  ...
  for (auto& closure : closures) {
    ...
    bool is_valid = validateClosurePose(query_points_icp, reference_points, refined_pose);
    if (!is_valid) {
      continue;
    }
...
bool Vegvisir::validateClosurePose(const std::vector<Eigen::Vector3d>& query_points,
  ...
  int intersection =
      static_cast<int>(static_cast<int>(num_source_voxels) + static_cast<int>(num_target_voxels) -
                       static_cast<int>(union_voxels));
  if (intersection < 0)
    intersection = 0;

  double overlap = static_cast<double>(intersection) /
...
std::vector<Eigen::Matrix4d> Vegvisir::fineGrainedOptimization() const {
  ...
  bool has_gnss = !gnss_measurements_.empty() || !gnss_pose_measurements_.empty();
  ...
  Eigen::Matrix<double, 6, 6> odom_info = odom_weight * Eigen::Matrix<double, 6, 6>::Identity();
  ...
  for (auto it = local_map_graph_.cbegin(); it != local_map_graph_.cend(); ++it) {
    ...
    if (first) {
      Eigen::Matrix4d first_pose = traj.empty() ? node.keypose() : node.keypose() * traj[0];
      ...
      Eigen::Matrix4d first_world = traj.empty() ? node.keypose() : node.keypose() * traj[0];
      ++id;
      pgo.addVariable(id, first_world);
      Eigen::Matrix4d inter_node_factor = prev_last_world_pose.inverse() * first_world;
  ...
  if (has_gnss) {
    const_cast<Vegvisir *>(this)->optimized_T_enu_map_ = pgo.getAlignmentTransform();
  }
