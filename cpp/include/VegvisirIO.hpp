...
struct MapMetadata {
  ...
  // Row k of keyposes.tum corresponds to map_id keypose_ids[k].
  std::vector<int> keypose_ids;
...
// Returns true on success, false on failure
bool savePosesTum(const std::string &file_path,
...
// reference_timestamps_ns carries the per-keypose timestamp recovered from
// column 1 of keyposes.tum; entries missing from the map default to 0.
